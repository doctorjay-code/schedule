...
export async function upsertLedgerRecord(record, fetchImpl = fetch) {
  const row = {
    id: String(record.id || ''),
    payment_method: record.payment || record.sheetName || record.payment_method || '기업카드',
    date: record.date,
    user_name: record.person || record.user_name || '기타',
    category: record.category || '',
    item: record.item || '항목 없음',
    memo: record.memo || '',
    fixed_cost: record.fixedCost !== undefined ? record.fixedCost : (record.fixed_cost || ''),
    type: (record.type || 'expense').toLowerCase(),
    amount: Number(record.amount || 0),
    balance: Number(record.balance || 0),
    order_index: record.orderIndex !== undefined ? record.orderIndex : (record.order_index ?? 0),
...
export async function insertLedgerRecordsBatch(records, fetchImpl = fetch) {
  ...
  const rows = records.map((record, index) => ({
    id: String(record.id || ''),
    payment_method: record.payment || record.payment_method || record.sheetName || '기업카드',
    date: record.date,
    user_name: record.person || record.user_name || '기타',
    category: record.category || '',
    item: record.item || '항목 없음',
    ...
    amount: Number(record.amount || 0),
    balance: Number(record.balance || 0),
    order_index: record.orderIndex !== undefined ? record.orderIndex : (record.order_index ?? ((index + 1) * 10)),
    offset_group_id: record.offset_group_id || null,
    offset_title: record.offset_title || null,
    is_forecast: Boolean(record.is_forecast),
    updated_at: now
